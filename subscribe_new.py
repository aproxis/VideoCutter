...
# Get main video duration using ffprobe
try:
    cmd = f"ffprobe -v error -show_entries format=duration -of json {input_video}"
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True, check=True)
    json_data = json.loads(result.stdout)
    main_duration = float(json_data['format']['duration'])
    print(f"Main video duration: {main_duration} seconds")
except Exception as e:
    print(f"Error getting main video duration: {e}")
    main_duration = 600  # Fallback to default duration if ffprobe fails
...
# Initialize base parts of the filter complex
audio_mix = f"[1:a]adelay={delay*1000}|{delay*1000},volume=0.5[a1];[0:a]volume=1.0[a0];[a0][a1]amix=inputs=2:normalize=0[aout];"
video_base = f"[0:v]setpts=PTS-STARTPTS[v0];[1:v]setpts=PTS-STARTPTS+{delay}/TB,chromakey=color=0x{chromakey_color}:similarity={chromakey_similarity}:blend={chromakey_blend}[ckout];"
overlay_base = f"[v0][ckout]overlay=enable='between(t\,{delay},{delay+overlay_duration})'"

# Add title text
title_text = f",drawtext=text='{title}':x=((w-tw)/2+{title_x_offset}):y=((h/2)+{title_y_offset}):enable='between(t\,{title_start},{title_end})':fontfile={fontfile}:fontsize={fontsize}:fontcolor=0x{fontcolor}:shadowcolor=black:shadowx=4:shadowy=2:alpha=0.8"

# Check if effect overlay is specified and exists
# Check if effect overlay is specified and exists
has_effect = False
if args.effect_overlay:
    effect_path = os.path.join('effects', args.effect_overlay)
    has_effect = os.path.exists(effect_path)
    if has_effect:
        print(f"Adding effect overlay: {effect_path}")
        if args.effect_blend.lower() in ["normal", "overlay", "over"]:
            # Use overlay filter with alpha simulated via colorchannelmixer
            filter_complex = (
                f"{audio_mix}"
                f"{video_base}"
                f"{overlay_base}[v1];"
                f"[2:v]format=rgba,colorchannelmixer=aa={args.effect_opacity},"
                f"setpts=PTS-STARTPTS,scale=iw:ih,setsar=1[effect];"
                f"[v1][effect]overlay=shortest=1{title_text}"
            )
        else:
            # Use blend with proper color conversion to rgb for correct modes like screen
            filter_complex = (
                f"{audio_mix}"
                f"{video_base}"
                f"{overlay_base}[v1];"
                f"[v1]format=rgba,scale=iw:ih,setsar=1,format=rgb24[v1rgb];"
                f"[2:v]format=rgba,trim=duration={main_duration},setpts=PTS-STARTPTS,"
                f"scale=iw:ih,setsar=1,format=rgb24[effect];"
                f"[v1rgb][effect]blend=all_mode={args.effect_blend}:all_opacity={args.effect_opacity}[blended];"
                f"[blended]format=yuv420p{title_text}"
            )
    else:
        print(f"Warning: Effect overlay file '{effect_path}' not found, continuing without effect overlay")
        filter_complex = f"{audio_mix}{video_base}{overlay_base}{title_text}"
else:
    # No effect specified
    filter_complex = f"{audio_mix}{video_base}{overlay_base}{title_text}"
...
# Filter complex is already complete at this point
...
# Add effect input if needed
if has_effect:
    overlay_command.extend(['-i', os.path.join('effects', args.effect_overlay)])
